...
class GnosisSupabaseUser extends BaseAuthUser {
  GnosisSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gnosisSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GnosisSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
